...
#else
    m_leftclick_tool_id =
        InsertPlugInTool(_T(""), _img_vfkaps, _img_vfkaps, wxITEM_CHECK,
                         _("VentureFarther Satellite Charts"), _T(""), NULL,
                         CALCULATOR_TOOL_POSITION, 0, this);
#endif
...
int vfkaps_pi::GetAPIVersionMajor() { return atoi(API_VERSION); }
...
int vfkaps_pi::GetPlugInVersionMajor() { return PLUGIN_VERSION_MAJOR; }

int vfkaps_pi::GetPlugInVersionMinor() { return PLUGIN_VERSION_MINOR; }

wxBitmap *vfkaps_pi::GetPlugInBitmap() { return &m_panelBitmap; }

wxString vfkaps_pi::GetCommonName() { return PLUGIN_API_NAME; }

wxString vfkaps_pi::GetShortDescription() { return PKG_SUMMARY; }

wxString vfkaps_pi::GetLongDescription() { return PKG_DESCRIPTION; }

int vfkaps_pi::GetToolbarToolCount(void) { return 1; }
...
void vfkaps_pi::ShowPreferencesDialog(wxWindow *parent) {
  ...
  if (Pref->ShowModal() == wxID_OK) {
    ...
    if (m_pDialog) {
      ...
      m_pDialog->m_stVFDownloadInfo1->SetLabel(
          _("Ready for multi-chart download"));

      m_pDialog->m_stVFDownloadInfo2->SetLabel(
          _("Ready for marker chart download"));
...
bool vfkaps_pi::RenderOverlay(wxDC &dc, PlugIn_ViewPort *vp) {
  if (!m_pDialog || !m_pDialog->IsShown() || !m_pOverlayFactory) return false;
